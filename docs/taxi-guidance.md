...
FAA AIM 4-3-18 and ICAO Doc 4444 require that an aircraft hold short of *every* runway it crosses on the way to its destination, with explicit ATC clearance for each crossing — controllers issue them one at a time. `TaxiGuidanceManager.InsertRunwayCrossingHoldShorts(route, destinationName, aircraftLat, aircraftLon)` runs after `TruncateToHoldShort` (which owns the destination runway's hold-short separately), inside the single shared `ApplyAutoHoldShortPasses` — which also writes the "Route crossings: ..." log line and strips the Progressive Taxi cleared-crossing runway. Both `LoadRoute` and `TryRecalculateRoute` call `ApplyAutoHoldShortPasses`; before 2026-09 only `LoadRoute` did, so an off-route recalculation silently produced a route with no crossing hold-shorts at all (PHNL 2026-09-03: a route to 04R via D tagged 26R/04L/04R at build time, a recalc 88 s later dropped all three, and the aircraft crossed all three at 13-19 kt with no callout). A route becomes the live route only through `AdoptRoute`, which runs `ApplyAutoHoldShortPasses` and then assigns `_route` — so the passes cannot be separated from the adoption they belong to. Assign `_route` there and nowhere else; the recalculation's call sits **below** its no-op guard, so a discarded recalc neither re-tags a route nobody adopts nor logs a crossings line claiming it did:

- For each route segment whose **edge** (FromNode→ToNode) crosses a runway centerline — `TaxiGraph.EdgeCrossesRunwayStatic` (a proper segment-vs-centerline-between-thresholds intersection) via `WhichRunwayCrossedByEdge` — tag the hold segment resolved by `RouteRunwayCrossings.ResolveCrossingHoldSegment` (see below) as `IsHoldShortPoint = true` with `HoldShortRunway = "runway X"`.
- Skip **only the route's own final arrival segment** on the destination strip — never every crossing named like the destination. `TruncateToHoldShort` owns that one segment; a blanket name-equality skip **dropped genuine mid-route crossings of the active runway**, leaving no hold-short before crossing it (see the 2026-08-24 entry below — do not restore it). The prefixed `"Runway 33L"` is normalized to bare `"33L"` (`RouteRunwayCrossings.StripRunwayPrefix`) and matched reciprocal-aware, because the crossing is named after whichever runway END is nearer the crossing point.
- Skip duplicate consecutive same-runway tags (`lastReportedRunway` — a wide runway's entry and exit edges both cross the centerline).
- **Report every crossing; place a hold only where one can go.** A crossing is always returned (so it reaches the route summary, the "Route changed" callout and the `crosses=` log line), but no hold is placed when there is no usable stop point: a crossing on the route's **first segment** (nothing earlier to hold on) or a candidate hold the aircraft has **already rolled past** (`HoldPointIsBehindAircraft`, 10 m tolerance). Tagging either commands a stop *on* the pavement — a recalculation is built from the live aircraft position and its start node can sit behind the aircraft, so a pilot cleared across a runway who has already crossed the hold line must not be handed that line again.
...
**WHERE the crossing hold-short lands: the scenery's own hold node, not "the node before the crossing edge"** (`RouteRunwayCrossings.ResolveCrossingHoldSegment`, shared by BOTH the auto-detector and the form's explicit per-row "Hold short of runway" picks in `ApplyUserRunwayHoldShorts` — they must resolve identically or one crossing gets two stop points). The crossing is detected against the runway **centerline**, and a taxi network routinely carries nodes ON the pavement (the crossing taxiway meets the runway's own path at a centerline node), so the node before the crossing edge is frequently *inside* the runway. Motivating case (LEBL 24R via D5, 2026-08): D5's nodes run 250 m → **105 m (HSND)** → 51 m → 21 m → centerline, and the old rule held the aircraft **21 m from the centerline of a 60 m-wide runway** — ~9 m inside the pavement edge, with the painted line 105 m back; the "Stop." cue (50 ft floor) then fired essentially at the runway edge. KBOS taxiway C over 04R lands at 25 m against a 25 m half-width, so the shape is not airport-specific.

The resolver walks BACKWARDS along the route from the crossing edge and takes the first segment ending at a `HoldShort` / `ILSHoldShort` node (navdata HS/HSND/IHS/IHSND) within `CrossingHoldLookbackMetres` (150 m), falling back to the old segment-before-the-crossing when the navdata carries no hold node there. Invariants:

- **The walk only ever moves the hold EARLIER along the route**, so a miss degrades to exactly the previous placement and can never park the aircraft closer to the runway than before. Do not "improve" this into a search that can move the hold forward.
- **First qualifying node wins** — i.e. the one nearest the runway, the full-length hold, matching a normal ATC crossing clearance. The CAT III / ILS hold preference stays a **destination-runway** opt-in (`TruncateToHoldShort` + the planner checkbox) and must not be extended to crossings.
- **Reciprocal-aware runway gate.** A hold node whose own `HoldShortName` names a *different* runway means the walk stepped past a junction onto another runway's line → stop and keep the fallback. The node is normally named after the *closer* threshold, so a crossing the user picked as "24R" legitimately meets a node named `runway 06L at D5` — the gate must compare against **both** designators (`Reciprocal`), or every mid-runway crossing rejects its own hold line.
- **Never walk THROUGH an already-tagged hold-short segment** — two runways must never share one stop point.
- This uses the navdata's own HS/HSND markers only. It is NOT the OSM `holding_position` sharpening that remains banned (taxi-data augmentation stays name-only and must never move hold-short geometry).
...
**The walk measures against the PAVEMENT frame
(`RunwayCenterline.Pavement*`), not the start-row one.** `RunwayCenterlines` are paired
from the navdata `start` table, and `SnapStartToRunwayCenterline` repairs those rows only
LATERALLY — the along-track position stays wherever the row sits, which at a displaced
threshold is hundreds of metres inside the pavement (LPPT 20: 626 m). `HalfWidthMeters`
is likewise a fixed 75 ft default. Measured over the shipped fs2024 DB, 7,123 of 95,989
runway ends (7.42 %) have a start row more than 50 m inboard and 344 exceed 400 m
outright, while 5,127 of 48,040 runways are wider than 150 ft — so a start-row-framed
"is this node ON the runway?" test rejects real runway pavement and reports an aircraft
standing on the runway as having no path to it. `Build` fills the `Pavement*` fields from
the runway table when it has one and falls back to the start-row values when it does not,
so the fields are ADDITIVE: every existing consumer of `Lat1`/`HalfWidthMeters`
(hold-short naming, `DescribeLocation`, crossing detection) keeps its tuned behaviour.
The entrance picker and the walk are the two halves of ONE reach test — they must agree
