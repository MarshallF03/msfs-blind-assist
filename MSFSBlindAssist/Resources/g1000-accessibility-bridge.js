// G1000 NXi Accessibility Bridge
// Injected into WTG1000_MFD.html via mod package override to expose GPS flight plan
// and navigation data to the MSFS Blind Assist application via HTTP on localhost:19778.
// Uses the SAME server port and protocol as the GNS 530 bridge — only one can be active.
//
// Runs in MSFS Coherent GT (older Chromium). No modern APIs like AbortSignal.timeout().
// Top-level try-catch ensures errors here never break the MFD.
try {

// Reuse the same _gps bridge object pattern as GNS 530
// If already loaded by GNS bridge, skip
if (typeof _gps !== 'undefined') {
    console.log('[G1000 Bridge] GPS bridge already loaded, skipping');
} else {
...
// --- HTTP Communication (identical to GNS 530 bridge) ---
    ...
    try {
        ...
        if (response.ok) {
            if (!_gps.serverConnected) {
                ...
                _gps.startPolling();
                _gps.postState('connected', { instrument: 'G1000_MFD' });
            }
            return true;
        }
    } catch (e) {
...
// --- Find the G1000 MFD instrument ---

_gps.findInstrument = function() {
    try {
        // G1000 MFD is registered as a custom element
        ...
        console.log('[G1000 Bridge] Error finding instrument: ' + e.message);
    }
    return null;
};
...
_gps.getFms = function() {
    ...
    try {
        ...
        if (inst) {
            ...
            _gps.fms = inst.fms;
            _gps.planner = inst.planner;
            console.log('[G1000 Bridge] FMS found');
            return _gps.fms;
        }
    } catch (e) {
        console.log('[G1000 Bridge] Error getting FMS: ' + e.message);
    }
    return null;
};

// --- Flight Plan State Extraction (identical data format to GNS 530) ---

_gps.sendFlightPlanState = function() {
    if (!_gps.serverConnected) return;
    try {
        var fms = _gps.getFms();
        if (!fms) { _gps.postState('no_fms'); return; }

        var planner = fms.flightPlanner;
        if (!planner || !planner.hasActiveFlightPlan()) {
            _gps.postState('flight_plan', { active: false, waypoints: [], activeLegIndex: -1, directTo: false });
            return;
        }
        ...
        for (var i = 0; i < plan.length; i++) {
            try {
                ...
                var wp = {
                    index: i, ident: '???', type: '', lat: 0, lon: 0,
                    distance: 0, dtk: 0, isActive: (i === activeLegIndex),
                ...
                if (leg.calculated) {
                    if (leg.calculated.endLat !== undefined) { wp.lat = leg.calculated.endLat; wp.lon = leg.calculated.endLon; }
                    if (leg.calculated.distanceWithTransitions !== undefined) wp.distance = leg.calculated.distanceWithTransitions;
                    if (leg.calculated.initialDtk !== undefined) wp.dtk = leg.calculated.initialDtk;
                }

                if (leg.flags !== undefined) wp.isMissedApproach = (leg.flags & 64) !== 0;

                waypoints.push(wp);
            } catch (legErr) { continue; }
        }

        var directToState = false, directToTarget = '';
        try {
            directToState = fms.getDirectToState() > 0;
            if (directToState) {
                directToTarget = fms.getDirectToTargetIcao() || '';
                if (directToTarget.length >= 7) directToTarget = directToTarget.substring(7).trim() || directToTarget.substring(3, 7).trim();
            }
        } catch (e) {}

        var approachInfo = { loaded: false, active: false, type: '', name: '', airport: '' };
        try {
        ...
        try {
            if (plan.originAirport) origin = plan.originAirport.substring(7).trim() || plan.originAirport.substring(3, 7).trim();
            if (plan.destinationAirport) destination = plan.destinationAirport.substring(7).trim() || plan.destinationAirport.substring(3, 7).trim();
        } catch (e) {}

        _gps.postState('flight_plan', {
            active: true, origin: origin, destination: destination,
            waypointCount: waypoints.length, activeLegIndex: activeLegIndex,
            waypoints: waypoints, directTo: directToState, directToTarget: directToTarget,
...
// --- Command Handling (identical to GNS 530) ---
...
_gps.handleCommand = function(command, payload) {
    try {
        switch (command) {
            ...
            case 'cancel_direct_to':
                if (_gps.fms) { _gps.fms.cancelDirectTo(); _gps.postState('command_ack', { command: command }); }
                break;
            case 'gps_drives_nav':
    ...
    } catch (e) { _gps.postState('command_error', { command: command, error: e.message }); }
};

// --- Initialization ---

_gps.init = function() {
    console.log('[G1000 Bridge] Initializing G1000 NXi accessibility bridge');
    _gps.tryConnect();
    setInterval(function() { if (!_gps.serverConnected) _gps.tryConnect(); }, _gps.RECONNECT_INTERVAL);
    var fmsCheckInterval = setInterval(function() {
        if (_gps.getFms()) {
            console.log('[G1000 Bridge] FMS ready');
            clearInterval(fmsCheckInterval);
            if (_gps.serverConnected) _gps.sendFlightPlanState();
        }
    }, 2000);
};
...
} // end if _gps not already defined
