...
/// <summary>
/// The vendor EFB (Shift+T) as a Page combo, a Tab combo and a list of the active page's rows
/// from coherent-c680-efb-agent.js: service cards and settings as "Label: on/off" or
/// "Label: A* / B", buttons as "[Label]", the rest as text. Enter on a card, setting or button
/// row acts on it (toggles the checkbox, steps the selector, presses the button) and the row is
/// re-read and spoken. F5 re-reads, Escape closes. One inspector socket: the window owns it.
/// </summary>
public sealed class C680EfbForm : Form
{
    ...
    public C680EfbForm(ScreenReaderAnnouncer announcer)
    {
        ...
        {
            Location = new Point(12, 44), Size = new Size(600, 520), Anchor = AnchorStyles.Top | AnchorStyles.Bottom | AnchorStyles.Left | AnchorStyles.Right,
            AccessibleName = "EFB page", TabIndex = 0
        };
        _list.SetText("Connecting to the EFB…");
        Controls.AddRange(new Control[] { pl, _page, tl, _tab, _list });
        ...
        _tab.SelectedIndexChanged += async (_, _) => { if (!_syncing && _tab.SelectedItem is string t) await Act($"__MSFSBA_C680_EFB.goTab({Js(t)})", t); };
        ...
        KeyDown += async (_, e) =>
        {
            ...
            if (e.KeyCode == Keys.Enter && _list.Focused && _list.SelectedIndex > 0)
            {
                ...
                if (row.StartsWith("[", StringComparison.Ordinal)) { await Act($"__MSFSBA_C680_EFB.act({i})", row.Trim('[', ']'), speakRow: i); return; }
                if (row.Contains(": ", StringComparison.Ordinal)) { await Act($"__MSFSBA_C680_EFB.act({i})", row, speakRow: i); return; }
    ...
    private async Task Act(string expr, string spoken, bool readTabs = false, int speakRow = -1)
    {
        if (_busy) return;
        _busy = true;
        try
        {
            ...
            if (speakRow >= 0 && speakRow < rows.Count) _announcer.AnnounceImmediate(rows[speakRow]);
        }
        finally { _busy = false; }
    }
