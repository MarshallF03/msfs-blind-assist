...
public partial class SimConnectManager
{
    ...
    /// GPS navigator that drives the stock GPS SimVars fills it (the Working Title Garmin
    /// SDK's GpsSynchronizer on the DA40's G1000 and the Learjet's GNS 530) — so it registers
    /// here with the fixed defs, ahead of the bulk per-aircraft registration and its ceiling.
    ///
    ...
    private void RegisterGpsWaypointDefinition()
    {
        ...
        {
            Log.Debug("SimConnect", $"GPS waypoint registration failed (waypoint and destination readouts unavailable): {ex.Message}");
        }
    }
